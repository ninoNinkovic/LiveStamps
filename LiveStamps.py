...
class Inject(sublime_plugin.TextCommand):
...
class LiveStampsHelp(sublime_plugin.TextCommand):
	...
	def run(self, view, info="time_flags"):
		...
		if info != False:
			self.view.run_command( "inject", { "data": info } )
	...
	def format_reference(self):
		...
		idx = len(max(flags.values(), key=len)) + 27
		...
		for flag in flags:
			...
			result[flag] = "| " +injection_flag.ljust(7) + " | " + injection_flag.format(value).ljust(idx) + " " + flags[flag]
	...
	def strftime_reference(self):
	 ...
	 # Get the longest line length
		...
		# Build the header
		header  = "|    | Output:                  | "+s.get("timezone")+": "+str(datetime.now())
		border  = "|----|--------------------------|"
		headstr = "{:<" + str(idx) +"}"
		bordstr = "{:-<"+ str(idx) +"}"
		...
		idx = len(time.strftime("%c"))

		# Calculate and concatenate
		for dn in map(chr, range(97, 123)):
			result[dn] = "| %" + dn + " | " + time.strftime("%"+dn).ljust(idx) + " | " + str(flags[dn])

			if dn != "%":
				up = dn.upper()
				result[up] = "| %" + up + " | " + time.strftime("%"+up).ljust(idx) + " | " + str(flags[up])

		# Format the info nicely
		return json.dumps(result, sort_keys=True, indent=2 )

	# Used for debugging stamp array
	def dump_meta(self):
...
class LiveStampsMenuGenCommand(sublime_plugin.TextCommand):
	...
	def run(self, edit, menutype="context"):
		...
		# Build a single menu
			...
			# Write the new menu
			self.write_menu(menuname, json.dumps( menujson, sort_keys=False, indent=2 ) )
	...
	def paste_menu(self, menu):
		self.view.run_command( "inject", { "data": menu } )
	...
	def add_keys(self, parent, keymap, noargs = False, sort = False):
	...
	def get_help_menu(self, header='Help', ID=False, mnemonic=False):
		...
		return self.add_keys(menu, {
			...
			"arg_keys" : {"info": 0},
			"cap_vals" : ["format_flags", "time_flags", "dump_meta"],
		}, False, True)

	# Submenu: Menu Generation
	def get_menugen_menu(self, header='Menu Generation', ID=False, mnemonic=False):
	...
	def get_preference_menu(self, header='Preferences', ID=False, mnemonic=False):
		...
		# Add divider and default keybindings commands, Special case: we create cap_vals programatically
		self.add_divider( menu )
